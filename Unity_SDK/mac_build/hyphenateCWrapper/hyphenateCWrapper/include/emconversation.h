...
public:
    ...
    /**
      * \brief Change all messages's isread and isacked status.
      *
      * @param  Change all messages's isread and isacked status.
      * @param  Exclude the messages which type are video, file or voice.
      * @return Return false if can't change isread and isacked status.
      */
    bool markAllSendMessagesAsReadAck(bool isRead = true, bool isNeedClickType = false);
    ...
    /**
      * \brief Load specified number of custom messages before the timestamp and contains the specified keywords from DB.
      *
      * Note: The result will be sorted by ASC.
      * @param  Message contains keywords, will ignore it if it's empty
      * @param  The reference timestamp, milliseconds, will reference current time if timestamp is negative
      ...
      * @return The loaded messages list.
      */
    EMMessageList loadMoreCustomMessages(const std::string& keywords, int64_t timeStamp = -1, int count = -1, const std::string &from = "", EMMessageSearchDirection direction = UP);
    ...
    /**
     * \brief Get conversation last sync roam key. if don't have, return empty string.
     */
    const std::string& lastSyncedMsgId() const;
...
private:
    ...
    std::shared_ptr<EMConversationPrivate> mPrivate;
};
