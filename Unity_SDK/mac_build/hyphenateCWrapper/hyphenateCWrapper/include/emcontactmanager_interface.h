...
public:
     ...
     * @param  username contact to be invited.
     */
    virtual void deleteContact(const std::string& username, EMError &error, bool keepConversation = false) = 0;
    ...
    /**
     * \brief get self id list on other platforms
     *
     * Note: sync operation
     * not save to local database
     * @return self id list
     */
    virtual std::vector<std::string> getSelfIdsOnOtherPlatform(EMError &error) = 0;
};
