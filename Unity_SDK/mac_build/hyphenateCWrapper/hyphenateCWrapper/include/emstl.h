...
//    EMMap<std::string, float> m1 = {{"", 10.2f}, {"", 10.0f}};      // ok
    ...
    public:
        ...
        std::recursive_mutex &getMutex() const { return mMutex; }
    protected:
    ...
    inline void synchronize(const EMSTLBase &obj, EMProcedure procedure) {
        EMSTLBase *temp = const_cast<EMSTLBase*>(&obj);
...
#define _lock_both(obj0, obj1)                                                              \
    ...
    public:
        ...
        //--------------------- constructor ---------------------
        explicit EMList (const allocator_type& alloc = allocator_type()) {
            _list = std::list<T>(alloc);
        }
        
        explicit EMList (size_type n, const value_type& val = value_type(),
                       const allocator_type& alloc = allocator_type()) {
            _list = std::list<T>(n, val, alloc);
        }
        
        template <class InputIterator>
        EMList (InputIterator first, InputIterator last,
              const allocator_type& alloc = allocator_type()) {
            _list = std::list<T>(first, last, alloc);
        }
        
        EMList (const std::list<T>& x) {
            _list = x;
        }
        
        EMList (const EMList& x) {
            _list = x._list;
        }
        ...
        inline const std::list<T> & get() const {
            return _list;
        }
        
        inline std::list<T> clone() const {
            EMList<T> *temp = const_cast<EMList<T>*>(this);
            std::lock_guard<std::recursive_mutex> lock(temp->getMutex());
            return _list;
        }
        ...
        EMList(std::initializer_list<value_type> __il) {
            _list = __il;
        }
        _LIBCPP_INLINE_VISIBILITY
        EMList(std::initializer_list<value_type> __il, const allocator_type& __a) {
            _list = std::list<value_type>(__il, __a);
        }
        _LIBCPP_INLINE_VISIBILITY
        inline EMList & operator=(std::initializer_list<value_type> &&__il) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list = std::list<value_type>(std::forward(__il));
            return *this;
        }
#endif  // _LIBCPP_HAS_NO_GENERALIZED_INITIALIZERS
        
        //--------------------- operator ---------------------
        inline EMList & operator=(const EMList<T> &v) {
            if (this == &v) {
                return *this;
            }
            EMList<T> *_v = const_cast<EMList<T>*>(&v);
            _lock_both(*this, *_v);
            _list = v.get();
            return *this;
        }
        
        inline EMList & operator=(const std::list<T> &v) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            if (&_list != &v) {
                _list = v;
            }
...
#ifndef _LIBCPP_HAS_NO_RVALUE_REFERENCES
        inline EMList & operator=(EMList<T> &&v) {
            _lock_both(*this, v);
            _list = std::move(v._list);
            return *this;
        }
        
        inline EMList & operator=(std::list<T> &&v) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list = std::move(v);
            return *this;
        }
        ...
        //--------------------- Operations ---------------------
        inline void splice (iterator position, std::list<T>& x) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list.splice(position, x);
        }
        
        inline void splice (iterator position, std::list<T>& x, iterator i) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list.splice(position, x, i);
        }
        
        inline void splice (iterator position, std::list<T>& x, iterator first, iterator last) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list.splice(position, x, first, last);
        }
        ...
        inline void unique (BinaryPredicate binary_pred) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list.unique(binary_pred);
        }
        
        inline void merge (std::list<T>& x) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list.merge(x);
        }
        
        template <class Compare>
        inline void merge (std::list<T>& x, Compare comp) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            _list.merge(x, comp);
        }
...
//        const value_type* data() const ;
    ...
    public:
        ...
        inline const_iterator find (const value_type& val) const {
            std::lock_guard<std::recursive_mutex> lock(const_cast<EMSet<value_type>*>(this)->getMutex());
            return _set.find(val);
        }
        ...
        inline const_iterator lower_bound (const value_type& val) const {
            std::lock_guard<std::recursive_mutex> lock(const_cast<EMSet<value_type>*>(this)->getMutex());
            return _set.lower_bound(val);
        }
        ...
        inline const_iterator upper_bound (const value_type& val) const {
            std::lock_guard<std::recursive_mutex> lock(const_cast<EMSet<value_type>*>(this)->getMutex());
            return _set.upper_bound(val);
        }
        
        inline std::pair<iterator,iterator> equal_range (const value_type& val) {
            std::lock_guard<std::recursive_mutex> lock(getMutex());
            return _set.equal_range(val);
        }
        
        inline std::pair<const_iterator,const_iterator> equal_range (const value_type& val) const {
            std::lock_guard<std::recursive_mutex> lock(const_cast<EMSet<value_type>*>(this)->getMutex());
            return _set.equal_range(val);
        }
