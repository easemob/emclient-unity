...
namespace ChatSDK
{
    public class GroupManager_Mac : IGroupManager
    {
        ...
        */
        ...
        {
            ChatAPINative.GroupManager_AcceptInvitationFromGroup(client, groupId, inviter,
        ...
        {
            ChatAPINative.GroupManager_DeclineInvitationFromGroup(client, groupId, username, reason,
                onSuccess: () => handle?.Success(),
                onError: (int code, string desc) => handle?.Error(code, desc));
        }
        ...
        {
            ChatAPINative.GroupManager_GetGroupsWithoutNotice(client, 
               onSuccessResult: (IntPtr[] data, DataType dType, int dSize) => {
                   List<string> muteList = new List<string>();
                   if (dType == DataType.String && dSize > 0)
                   {
        ...
        {
            //to-do: API missing some parameters?
            string nickName = "";
            string message = "";
            ChatAPINative.GroupManager_ApplyJoinPublicGroup(client, groupId, nickName, message,
               onSuccess: () => handle?.Success(),
               onError: (int code, string desc) => handle?.Error(code, desc));
        }
